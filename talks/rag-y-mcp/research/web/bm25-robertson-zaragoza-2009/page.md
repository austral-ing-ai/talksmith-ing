...
c(cid:1) 2009 S. Robertson and H. Zaragoza
...
Ei1(tf ) := P (TF i = tf | Ei = elite); Ei0(tf ) := P (TF i = tf | Ei = elite)
...
ture, where common choices are tf
itself and (1 + log tf ). The latter
...
(cid:9)avdl = average of (cid:8)dl across documents
(cid:8)tf i
(1 − b) + b
...
can integrate non-textual
features very easily into the PRF and
...
such as BM25F(q, d)+log(P ageRank(d)) may work well in practice for
...
the log into a sigmoid and tuning the two extra parameters λ(cid:5) and λ(cid:5)(cid:5)
...
N +0.5
of writing this survey. For example, PF/Tijah uses idf= log
...
The DFR models [2],
like the language models, do not contain
...
Parameter optimisation comes with considerable costs:
...
dent searches, to ﬁnd the optimum value θ(cid:5)
...
eters linearly from θ to θ(cid:5). Finally we choose the best parameter setting
...
K1 Scaling: Note that in general k1 will depend on the weights
...
“Trust Region Algorithms,” Webpage, http://www.
...
search in an XML database system,” Proceedings of
the 2nd Interna-
