...
Portada. Presentate y presentá a los otros dos docentes.
...
Retiradas el 2026-09-04. Motivo: el Talk se importó de otra materia donde era la clase 2 y estos temas no se habían dado. Acá cae en la clase 8, después de dos clases enteras de redes neuronales. Se reemplazaron por una sola lámina de repaso.
...
## Las cuatro láminas de entrenamiento de la sección 5 (5.4 a 5.7)

Retiradas el 2026-09-04 por pedido del presentador. Motivo: las cuatro repiten material que la
clase de **modelado de redes neuronales** ya da entero, y con más detalle. Esa clase va antes en
la materia, así que acá se estaban dando por segunda vez.

- **5.4 "Bajar por la pendiente"** — la superficie de pérdida, la dirección −∇ y la analogía de la
  montaña con niebla. Está en 6.1 de modelado, "Buscar el mínimo de una función", que además
  dibuja el punto de arranque al azar, los pasos que se acortan cerca del fondo y el mínimo.
- **5.5 "El learning rate es el paso"** — los dos modos de falla del mismo número. Está en 6.9 de
  modelado, "La tasa de aprendizaje η". El diagrama de las dos curvas de loss se movió allá y
  reemplazó a la imagen de fórmula que esa lámina traía.
- **5.6 "Cuatro pasos que se repiten"** — el bucle cerrado del entrenamiento. Está repartido entre
  6.3 de modelado, "Entrenar es un ciclo de dos movimientos", y 6.11, "El ciclo completo, batch a
  batch", que sí distinguen lo que se acumula dentro del batch de lo que se aplica al cerrarlo.
- **5.7 "Repartir la culpa hacia atrás"** — ida, vuelta y la malla ampliada. Es la sección 6
  completa de modelado, trece láminas que van del δ de salida a la regla de la cadena en
  movimiento.

Con esto la sección 5 queda en tres láminas y hace una sola cosa: construir la red desde el
perceptrón hasta el problema del orden, que es lo que la sección 6 necesita para abrir. La tesis
soltó "gradientes" de su enumeración y el arco de la agenda dice explícitamente que el ajuste de
pesos se da por visto.

Las imágenes de las cuatro láminas siguen en `images/`
(`superficie-de-perdida.jpg`, `s5-5-1-learning-rate-paso.*`, `s5-6-1-ciclo-entrenamiento.*`,
`s5-7-1-*`). El texto completo de las cuatro está en el historial de git de este archivo.
