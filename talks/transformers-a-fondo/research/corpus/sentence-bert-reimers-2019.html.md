---
source_file: sentence-bert-reimers-2019.html
source_type: article
...
## Provenance
- Original location: research/raw/sentence-bert-reimers-2019.html
- Format: html (ar5iv render of arXiv 1908.10084, 144 KB)
- Author / source (if known): Reimers, Gurevych — UKP Lab, TU Darmstadt; EMNLP 2019
- Date of original (if known): 2019-08
- Coverage in this record: complete paper (§1–8), all tables. Bibliography stripped.

## Key claims
- Problem: BERT/RoBERTa solve sentence-pair tasks by feeding both sentences into one network (cross-encoder), so "finding the most similar pair in a collection of 10,000 sentences requires about 50 million inference computations (~65 hours)"; and BERT's raw outputs make poor sentence embeddings — averaging BERT token embeddings (54.81 avg STS) or using the CLS vector (29.19) is **worse than averaging GloVe** (61.32) (Table 1).
- SBERT: add a **pooling** layer to BERT/RoBERTa (MEAN by default; CLS and MAX tested) and fine-tune with **siamese / triplet** networks so that embeddings are comparable by cosine similarity. Three objectives: classification $o=\mathrm{softmax}(W_t(u,v,|u-v|))$; regression on $\cos(u,v)$ with MSE; triplet $\max(\|s_a-s_p\|-\|s_a-s_n\|+\epsilon,0)$ with $\epsilon=1$.
- Training on SNLI + MultiNLI (570k + 430k pairs), 1 epoch, batch 16, Adam lr 2e-5, warmup 10%.
- Results: unsupervised STS average 74.89 (SBERT-NLI-base) / 76.55 (large) vs 71.22 Universal Sentence Encoder (Table 1); supervised STSb up to 90.49 with NLI pre-training then STSb fine-tuning vs 90.15 BERT cross-encoder (Table 2); SentEval transfer better than InferSent and USE on 5 of 7 tasks (Table 6).
- Ablation (Table 7): MEAN pooling best; the $|u-v|$ feature is the most important part of the classification objective; adding $u*v$ hurts slightly.
- Efficiency (§7, Table 7-speed): most-similar-pair search drops "from 65 hours … to about 5 seconds"; SBERT-base with smart batching 2042 sentences/s on a V100 GPU vs 1318 USE and 1876 InferSent; on CPU InferSent is faster (single BiLSTM vs 12 Transformer layers).

## Definitions and terminology
- **Cross-encoder** (both sentences into one BERT pass) vs **bi-encoder / siamese network** (each sentence encoded independently, tied weights).
- **Pooling strategies**: CLS, MEAN, MAX.
- **Classification / regression / triplet objective functions**; **anchor / positive / negative** sentence; **margin** $\epsilon$.
- **STS** (semantic textual similarity, Spearman $\rho$ of cosine vs gold); **SentEval** transfer tasks; **smart batching** (group by length, pad to the longest in the mini-batch).

## Evidence and examples
- Table 1: Spearman ρ×100 on STS12–16, STSb, SICK-R for GloVe avg, BERT avg, BERT CLS, InferSent, USE, SBERT/SRoBERTa base/large.
- Table 2: supervised STSb (BERT cross-encoder 84.30 / 90.15 vs SBERT 84.67 / 88.33 / 90.49 with NLI pre-training).
- Table 3: Argument Facet Similarity (cross-topic drop from 84.3 to ~70 for BERT, SBERT 76.57 cross-topic); Table 4: Wikipedia sections triplet accuracy (SBERT-WikiSec-large 80.78); Table 5: SentEval; Table 6: pooling/concatenation ablation; Table 7: speed CPU/GPU.
- Figures 1–2: siamese architecture with classification objective (training) and cosine-similarity (inference).

## Inconsistencies / open questions
- The 65 h → 5 s comparison assumes 10k sentences on a V100; the argument is about $O(n^2)$ cross-encoder passes vs $n$ bi-encoder passes plus cheap cosine.
- Later work (contrastive training, MNR loss, larger corpora) supersedes the numbers; the architecture idea (pool + fine-tune bi-encoder) is what matters for the RAG practice encoder choice.

## Images / diagrams
Companion folder `sentence-bert-reimers-2019.html/images/` is **empty**: the source is an ar5iv render whose figures are remote assets, and the orchestrator asked for no new fetches. Each figure is listed below with its caption (captions are also inline in the excerpts) so the diagram-illustrator can redraw from the description if a slide needs it.

### Image 1
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 1: SBERT architecture with classification objective function, e.g., for fine-tuning on SNLI dataset. The two BERT networks have tied weights (siamese network structure).
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 2
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 2: SBERT architecture at inference, for example, to compute similarity scores. This architecture is also used with the regression objective function.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 3
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 1: Spearman rank correlation $\rho$ between the cosine similarity of sentence representations and the gold labels for various Textual Similarity (STS) tasks. Performance is reported by convention as $\rho\times 100$. STS12-STS16: SemEval 2012-2016, STSb: STSbenchmark, SICK-R: SICK relatedness dataset.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 4
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 2: Evaluation on the STS benchmark test set. BERT systems were trained with 10 random seeds and 4 epochs. SBERT was fine-tuned on the STSb dataset, SBERT-NLI was pretrained on the NLI datasets, then fine-tuned on the STSb dataset.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 5
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 3: Average Pearson correlation $r$ and average Spearman’s rank correlation $\rho$ on the Argument Facet Similarity (AFS) corpus Misra et al. 2016. Misra et al. proposes 10-fold cross-validation. We additionally evaluate in a cross-topic scenario: Methods are trained on two topics, and are evaluated on the third topic.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 6
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 4: Evaluation on the Wikipedia section triplets dataset Dor et al. 2018. SBERT trained with triplet loss for one epoch.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 7
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 5: Evaluation of SBERT sentence embeddings using the SentEval toolkit. SentEval evaluates sentence embeddings on different sentence classification tasks by training a logistic regression classifier using the sentence embeddings as features. Scores are based on a 10-fold cross-validation.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 8
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 6: SBERT trained on NLI data with the classification objective function, on the STS benchmark (STSb) with the regression objective function. Configurations are evaluated on the development set of the STSb using cosine-similarity and Spearman’s rank correlation. For the concatenation methods, we only report scores with `MEAN` pooling strategy.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 9
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 7: Computation speed (sentences per second) of sentence embedding methods. Higher is better.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)
...
We evaluate SBERT and SRoBERTa on common STS tasks and transfer learning tasks, where it outperforms other state-of-the-art sentence embeddings methods. [footnote: ^1^

Code available: https://github.com/UKPLab/sentence-transformers]

## 1 Introduction

In this publication, we present Sentence-BERT (SBERT), a modification of the BERT network using siamese and triplet networks that is able to derive semantically meaningful sentence embeddings [footnote: ^2^
...
We train SBERT on the combination of the SNLI Bowman et al. 2015 and the Multi-Genre NLI Williams et al. 2018 dataset. The SNLI is a collection of 570,000 sentence pairs annotated with the labels *contradiction*, *eintailment*, and *neutral*. MultiNLI contains 430,000 sentence pairs and covers a range of genres of spoken and written text. We fine-tune SBERT with a 3-way softmax-classifier objective function for one epoch. We used a batch-size of 16, Adam optimizer with learning rate $2\mathrm{e}{-5}$, and a linear learning rate warm-up over 10% of the training data. Our default pooling strategy is `MEAN`.
...
For our comparison we use the sentences from the STS benchmark Cer et al. 2017. We compute average GloVe embeddings using a simple for-loop with python dictionary lookups and NumPy. InferSent [footnote: ^4^

https://github.com/facebookresearch/InferSent] is based on PyTorch. For Universal Sentence Encoder, we use the TensorFlow Hub version [footnote: ^5^

https://tfhub.dev/google/universal-sentence-encoder-large/3] , which is based on TensorFlow. SBERT is based on PyTorch. For improved computation of sentence embeddings, we implemented a smart batching strategy: Sentences with similar lengths are grouped together and are only padded to the longest element in a mini-batch. This drastically reduces computational overhead from padding tokens.
