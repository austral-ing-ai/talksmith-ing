---
source_file: bert-devlin-2018.html
source_type: article
...
## Provenance
- Original location: research/raw/bert-devlin-2018.html
- Format: html (ar5iv render of arXiv 1810.04805, 235 KB)
- Author / source (if known): Devlin, Chang, Lee, Toutanova — Google AI Language; NAACL 2019
- Date of original (if known): 2018-10 (v1), 2019-05 (v2)
- Coverage in this record: complete main text (§1–6) and appendices A–C, equations as LaTeX, all tables. Bibliography stripped.

## Key claims
- BERT is "a multi-layer bidirectional Transformer encoder based on the original implementation described in Vaswani et al. 2017"; i.e. an **encoder-only** Transformer. Footnote 4: "the bidirectional Transformer is often referred to as a 'Transformer encoder' while the left-context-only version is referred to as a 'Transformer decoder' since it can be used for text generation."
- Sizes: BERT-BASE L=12, H=768, A=12, 110M parameters; BERT-LARGE L=24, H=1024, A=16, 340M. Feed-forward size $4H$ (3072 / 4096). BASE was sized to match OpenAI GPT.
- Two pre-training tasks: **Masked LM** (15% of WordPiece tokens chosen; of those 80% → [MASK], 10% → random token, 10% unchanged; predict the original with cross-entropy) and **Next Sentence Prediction** (50% real next sentence, 50% random; binary label from the final [CLS] vector $C$).
- Input = token embeddings (WordPiece, 30k vocab) + segment embeddings (A/B) + position embeddings; [CLS] first, [SEP] between sentences; max 512 tokens.
- Fine-tuning: same architecture, add one output layer, fine-tune all parameters end-to-end per task; cheap ("at most 1 hour on a single Cloud TPU, or a few hours on a GPU").
- Pre-training data: BooksCorpus (800M words) + English Wikipedia (2,500M words); 1M steps, batch 256 sequences × 512 tokens (≈ 40 epochs over 3.3B words); Adam lr 1e-4, warmup 10k steps, dropout 0.1, GELU; 4 days on 4 Cloud TPUs (BASE) or 16 TPUs (LARGE).
- Results: GLUE 80.5 (LARGE; +7.7 abs over prior), SQuAD v1.1 F1 93.2, SQuAD v2.0 F1 83.1, SWAG 86.3; ablations show removing NSP or using left-to-right only hurts, and bigger models help even on small tasks.
- Feature-based use (§5.3): concatenating the last four hidden layers as frozen features gets 96.1 F1 on CoNLL-NER, only 0.3 behind fine-tuning — the basis for using BERT as an embedding backbone (relevant to the RAG practice).

## Definitions and terminology
- **Bidirectional** vs **left-to-right** / **constrained** self-attention.
- **Masked LM (MLM)** / "Cloze task"; **Next Sentence Prediction (NSP)**.
- **[CLS]**: special first token whose final hidden state $C\in\mathbb{R}^H$ is the aggregate sequence representation; **[SEP]**: separator; **segment embedding**.
- "Sentence" = arbitrary span of contiguous text; "sequence" = the input token sequence (one or two sentences packed).
- Notation: $L$ layers, $H$ hidden size, $A$ attention heads, $E$ input embedding, $T_i$ final hidden vector of token $i$.
- **Feature-based** vs **fine-tuning** approaches to transfer.

## Evidence and examples
- Table 1 GLUE results (BERT-BASE 79.6, BERT-LARGE 82.1 average, vs OpenAI GPT 75.1); Table 2 SQuAD 1.1; Table 3 SQuAD 2.0; Table 4 SWAG; Table 5 ablation of pre-training tasks (No NSP, LTR & No NSP, + BiLSTM); Table 6 effect of model size; Table 7 CoNLL-2003 NER feature-based results (concat last four layers 96.1 F1); Table 8 masking-strategy ablation.
- Figure 1 (pre-training vs fine-tuning), Figure 2 (input representation as sum of three embeddings), Figure 3 (comparison with OpenAI GPT and ELMo), Figure 4 (fine-tuning on the four task types), Figure 5 (effect of training steps).

## Inconsistencies / open questions
- The paper does not give a per-component parameter breakdown; 110M ≈ embeddings (30522×768 ≈ 23M) + 12 blocks × ~7.1M each, which the class can derive from the scaling-laws formula.
- NSP's usefulness was later contested (RoBERTa); the paper claims it matters (Table 5).
- Relation to the class: BERT is the encoder used in the RAG practice; Sentence-BERT (separate record) explains why raw BERT token vectors make poor sentence embeddings.

## Images / diagrams
Companion folder `bert-devlin-2018.html/images/` is **empty**: the source is an ar5iv render whose figures are remote assets, and the orchestrator asked for no new fetches. Each figure is listed below with its caption (captions are also inline in the excerpts) so the diagram-illustrator can redraw from the description if a slide needs it.

### Image 1
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 1: Overall pre-training and fine-tuning procedures for BERT. Apart from output layers, the same architectures are used in both pre-training and fine-tuning. The same pre-trained model parameters are used to initialize models for different down-stream tasks. During fine-tuning, all parameters are fine-tuned. `[CLS]` is a special symbol added in front of every input example, and `[SEP]` is a special separator token (e.g. separating questions/answers).
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 2
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 2: BERT input representation. The input embeddings are the sum of the token embeddings, the segmentation embeddings and the position embeddings.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 3
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 1: GLUE Test results, scored by the evaluation server (https://gluebenchmark.com/leaderboard). The number below each task denotes the number of training examples. The ‘‘Average’’ column is slightly different than the official GLUE score, since we exclude the problematic WNLI set. [footnote: ^9^  See question 10 in https://gluebenchmark.com/faq.] BERT and OpenAI GPT are single-model, single task. F1 scores are reported for QQP and MRPC, Spearman correlations are reported for STS-B, and accuracy scores are reported for the other tasks. We exclude entries that use BERT as one of their components.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 4
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 2: SQuAD 1.1 results. The BERT ensemble is 7x systems which use different pre-training checkpoints and fine-tuning seeds.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 5
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 3: SQuAD 2.0 results. We exclude entries that use BERT as one of their components.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 6
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 4: SWAG Dev and Test accuracies. ^†^Human performance is measured with 100 samples, as reported in the SWAG paper.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 7
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 5: Ablation over the pre-training tasks using the BERT${}_{\textsc{BASE}}$ architecture. “No NSP” is trained without the next sentence prediction task. “LTR & No NSP” is trained as a left-to-right LM without the next sentence prediction, like OpenAI GPT. “+ BiLSTM” adds a randomly initialized BiLSTM on top of the “LTR + No NSP” model during fine-tuning.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 8
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 6: Ablation over BERT model size. #L = the number of layers; #H = hidden size; #A = number of attention heads. “LM (ppl)” is the masked LM perplexity of held-out training data.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 9
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 7: CoNLL-2003 Named Entity Recognition results. Hyperparameters were selected using the Dev set. The reported Dev and Test scores are averaged over 5 random restarts using those hyperparameters.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 10
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 3: Differences in pre-training model architectures. BERT uses a bidirectional Transformer. OpenAI GPT uses a left-to-right Transformer. ELMo uses the concatenation of independently trained left-to-right and right-to-left LSTMs to generate features for downstream tasks. Among the three, only BERT representations are jointly conditioned on both left and right context in all layers. In addition to the architecture differences, BERT and OpenAI GPT are fine-tuning approaches, while ELMo is a feature-based approach.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 11
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 4: Illustrations of Fine-tuning BERT on Different Tasks.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 12
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Figure 5: Ablation over number of training steps. This shows the MNLI accuracy after fine-tuning, starting from model parameters that have been pre-trained for $k$ steps. The x-axis is the value of $k$.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)

### Image 13
- Filename: *(not on disk)*
- Provenance: table/algorithm rendered inline (no bitmap)
- Caption (verbatim): Table 8: Ablation over different masking strategies.
- Depiction / Why it matters / Transcribed text: not transcribed (process_images: no; bytes not fetched)
...
BERT’s model architecture is a multi-layer bidirectional Transformer encoder based on the original implementation described in Vaswani et al. 2017 and released in the `tensor2tensor` library. [footnote: ^1^

https://github.com/tensorflow/tensor2tensor] Because the use of Transformers has become common and our implementation is almost identical to the original, we will omit an exhaustive background description of the model architecture and refer readers to Vaswani et al. 2017 as well as excellent guides such as ‘‘The Annotated Transformer.’’ [footnote: ^2^

http://nlp.seas.harvard.edu/2018/04/03/attention.html]

In this work, we denote the number of layers (i.e., Transformer blocks) as $L$, the hidden size as $H$, and the number of self-attention heads as $A$. [footnote: ^3^

In all cases we set the feed-forward/filter size to be $4H$, i.e., 3072 for the $H=768$ and 4096 for the $H=1024$.] We primarily report results on two model sizes: **BERT${}_{\textsc{BASE}}$** (L=12, H=768, A=12, Total Parameters=110M) and **BERT${}_{\textsc{LARGE}}$** (L=24, H=1024, A=16, Total Parameters=340M).

BERT${}_{\textsc{BASE}}$ was chosen to have the same model size as OpenAI GPT for comparison purposes. Critically, however, the BERT Transformer uses bidirectional self-attention, while the GPT Transformer uses constrained self-attention where every token can only attend to context to its left. [footnote: ^4^
...
input embedding as $E$, the final hidden vector of the special `[CLS]` token as $C\in\mathbb{R}^{H}$, and the final hidden vector for the $i^{\rm th}$ input token as $T_{i}\in\mathbb{R}^{H}$.
...
(labeled as `NotNext`).
As we show in Figure 1, $C$ is used for next sentence prediction (NSP). [footnote: ^5^
...
Compared to pre-training, fine-tuning is relatively inexpensive. All of the results in the paper can be replicated in at most 1 hour on a single Cloud TPU, or a few hours on a GPU, starting from the exact same pre-trained model. [footnote: ^7^
...
> **Table 1: GLUE Test results, scored by the evaluation server (https://gluebenchmark.com/leaderboard). The number below each task denotes the number of training examples. The ‘‘Average’’ column is slightly different than the official GLUE score, since we exclude the problematic WNLI set. [footnote: ^9^  See question 10 in https://gluebenchmark.com/faq.] BERT and OpenAI GPT are single-model, single task. F1 scores are reported for QQP and MRPC, Spearman correlations are reported for STS-B, and accuracy scores are reported for the other tasks. We exclude entries that use BERT as one of their components.**
...
^9^^9^footnotetext: See (10) in https://gluebenchmark.com/faq.
...
| Ours |  |  |  |  |
| BERT${}_{\textsc{BASE}}$ (Single) | 80.8 | 88.5 | - | - |
| BERT${}_{\textsc{LARGE}}$ (Single) | 84.1 | 90.9 | - | - |
| BERT${}_{\textsc{LARGE}}$ (Ensemble) | 85.8 | 91.8 | - | - |
| BERT${}_{\textsc{LARGE}}$ (Sgl.+TriviaQA) | **84.2** | **91.1** | **85.1** | **91.8** |
| BERT${}_{\textsc{LARGE}}$ (Ens.+TriviaQA) | **86.2** | **92.2** | **87.4** | **93.2** |
...
| Ours |  |  |  |  |
| BERT${}_{\textsc{LARGE}}$ (Single) | 78.7 | 81.9 | 80.0 | 83.1 |
...
The top results from the SQuAD leaderboard do not have up-to-date public system descriptions available, [footnote: ^12^

QANet is described in Yu et al. 2018, but the system has improved substantially after publication.] and are allowed to use any public data when training their systems. We therefore use modest data augmentation in our system by first fine-tuning on TriviaQA Joshi et al. 2017 befor fine-tuning on SQuAD.

Our best performing system outperforms the top leaderboard system by +1.5 F1 in ensembling and +1.3 F1 as a single system. In fact, our single BERT model outperforms the top ensemble system in terms of F1 score. Without TriviaQA fine-tuning data, we only lose 0.1-0.4 F1, still outperforming all existing systems by a wide margin. [footnote: ^13^
...
|  | (Acc) | (Acc) | (Acc) | (Acc) | (F1) |
| BERT${}_{\textsc{BASE}}$ | 84.4 | 88.4 | 86.7 | 92.7 | 88.5 |
...
The advantage of this procedure is that the Transformer encoder does not know which words it will be asked to predict or which have been replaced by random words, so it is forced to keep a distributional contextual representation of *every* input token. Additionally, because random replacement only occurs for 1.5% of all tokens (i.e., 10% of 15%), this does not seem to harm the model’s language understanding capability. In Section C.2,
...
$$Input \displaystyle=\text{\tt{[CLS] the man went to [MASK] store [SEP]}} \\
he bought a gallon [MASK] milk [SEP] \\
Label \displaystyle=\text{\tt{IsNext}} \\
Input \displaystyle=\text{\tt{[CLS] the man [MASK] to the store [SEP]}} \\
penguin [MASK] are flight ##less birds [SEP] \\
Label \displaystyle=\text{\tt{NotNext}}$$
...
Training of BERT${}_{\textsc{BASE}}$ was performed on 4 Cloud TPUs in Pod configuration (16 TPU chips total). [footnote: ^14^

https://cloudplatform.googleblog.com/2018/06/Cloud-TPU-now-offers-preemptible-pricing-and-global-availability.html] Training of BERT${}_{\textsc{LARGE}}$ was performed on 16 Cloud TPUs (64 TPU chips total). Each pre-training took 4 days to complete.
...
architectures are shown visually in Figure 3. Note that
...
Our GLUE results in Table9 are obtained from https://gluebenchmark.com/leaderboard and https://blog.openai.com/language-unsupervised.
...
Recognizing Textual Entailment is a binary entailment task similar to MNLI, but with much less training data Bentivogli et al. 2009. [footnote: ^15^

Note that we only report single-task fine-tuning results in this paper. A multitask fine-tuning approach could potentially push
...
Winograd NLI is a small natural language inference dataset Levesque et al. 2011. The GLUE webpage notes that there are issues with the construction of this dataset,  [footnote: ^16^

https://gluebenchmark.com/faq] and every trained system that’s been submitted to GLUE has performed worse than the 65.1 baseline accuracy of predicting the majority class. We therefore exclude this set to be fair to OpenAI GPT. For our GLUE submission, we always predicted the majority class.
...
In Section 3.1, we mention that BERT uses a mixed strategy for masking the target tokens when pre-training with the masked language
...
between pre-training and fine-tuning, as the `[MASK]` symbol never appears during the fine-tuning stage. We report the Dev results for both
...
Mask means that we replace the target token with the `[MASK]` symbol
...
was shown to be the best approach in Section 5.3.
