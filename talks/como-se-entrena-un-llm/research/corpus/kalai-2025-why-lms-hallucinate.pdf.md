---
source_file: kalai-2025-why-lms-hallucinate.pdf
source_type: article
...
## Provenance
- Original location: articles/kalai-2025-why-lms-hallucinate.pdf
- Format: pdf (36 pages)
- Author / source (if known): Adam Tauman Kalai∗ (OpenAI; adam@kal.ai), Santosh S. Vempala† (Georgia Tech; supported in part by NSF award CCF-2106444 and a Simons Investigator award; vempala@gatech.edu), Ofir Nachum (OpenAI), Edwin Zhang (OpenAI). arXiv preprint, cs.CL.
- Date of original (if known): September 4, 2025 (arXiv:2509.04664v1, 4 Sep 2025). Model queries cited in the paper were made 9–11 May 2025; leaderboards accessed 24–27 June 2025.
- arXiv: 2509.04664v1 [cs.CL]
...
## Key claims
- Core argument (Abstract): "Like students facing hard exam questions, large language models sometimes guess when uncertain, producing plausible yet incorrect statements instead of admitting uncertainty." — "language models hallucinate because the training and evaluation procedures reward guessing over acknowledging uncertainty".
- Origin in pretraining: "Hallucinations need not be mysterious—they originate simply as errors in binary classification. If incorrect statements cannot be distinguished from facts, then hallucinations in pretrained language models will arise through natural statistical pressures."
- Persistence after post-training: "hallucinations persist due to the way most evaluations are graded—language models are optimized to be good test-takers, and guessing when uncertain improves test performance."
- Proposed fix is socio-technical: the "“epidemic” of penalizing uncertain responses can only be addressed through a socio-technical mitigation: modifying the scoring of existing benchmarks that are misaligned but dominate leaderboards, rather than introducing additional hallucination evaluations."
- Hallucination = "overconfident, plausible falsehoods"; the term "differs fundamentally from the human perceptual experience". Examples: DeepSeek-V3 gave three different wrong birthdays for Adam Tauman Kalai ("03-07", "15-06", "01-01") despite being told to answer only if known; ChatGPT (GPT-4o), DeepSeek and Llama gave three different wrong dissertation titles/years.
- "even if the training data were error-free, the objectives optimized during language model training would lead to errors being generated"; realistic noisy data would give even higher error rates.
- The analysis is general: it "applies broadly, including to reasoning and search-and-retrieval language models, and the analysis does not rely on properties of next-word prediction or Transformer-based neural networks"; it considers only pretraining and post-training.
- Key reduction: generating valid outputs is harder than classifying validity (the Is-It-Valid, IIV, problem): "(generative error rate) ≳ 2 · (IIV misclassification rate)".
- Same statistical factors that cause misclassification cause LM errors: arbitrary facts with no pattern (epistemic uncertainty, e.g., birthdays), poor models (e.g., letter counting, trigram grammar), computational hardness, distribution shift, and GIGO (garbage in, garbage out).
- Singleton-rate bound: "the hallucination rate, after pretraining, should be at least the fraction of training facts that appear once. For instance, if 20% of birthday facts appear exactly once in the pretraining data, then one expects base models to hallucinate on at least 20% of birthday facts." Builds on Turing's missing-mass (Good–Turing) estimator. "Large language models seldom err on frequently referenced facts, e.g., Einstein’s birthday".
- Calibration and errors are linked: base models trained with cross-entropy tend to be calibrated (small δ), and "language models which do not err must not be calibrated"; "calibration—and, hence, errors—is a natural consequence of the standard cross-entropy objective." Post-trained models may deviate from calibration (Figure 2: GPT-4 before/after RL).
- "Hallucinations are inevitable only for base models": a non-hallucinating system is easy to build (question-answer database + calculator, otherwise IDK), so hallucination is not inevitable in general.
- Not merely autocomplete: "the autocomplete view of language models is no more significant than the fact that any human speaker produces one word at a time"; errors arise from fitting the language distribution.
- Letter counting as a poor-model issue: "How many Ds are in DEEPSEEK?" — DeepSeek-V3 answered "2" or "3" in ten trials (Meta AI and Claude 3.7 Sonnet similar, up to "6" and "7"), while DeepSeek-R1 reliably counts by spelling D-E-E-P-S-E-E-K; tokenization (D/EEP/SEE/K) is a representational challenge that reasoning overcomes.
- Post-training reduces some GIGO errors (misconceptions, conspiracy theories) — RLHF, RLAIF, DPO shown to reduce hallucinations — but "further reduction of hallucinations is an uphill battle, since existing benchmarks and leaderboards reinforce certain types of hallucination."
- Binary grading: "Under binary grading, abstaining is strictly sub-optimal. IDK-type responses are maximally penalized while an overconfident “best guess” is optimal." Observation 1: for any beliefs over binary graders, "the optimal response(s) are not abstentions".
- Model A vs Model B: a model that "correctly signals uncertainty and never hallucinates" (A) is outperformed under 0-1 scoring by a model that "always “guesses” when unsure" (B) — "This creates an “epidemic” of penalizing uncertainty and abstention".
- Language models "are always in “test-taking” mode"; "Humans learn the value of expressing uncertainty outside of school, in the school of hard knocks." "Put simply, most evaluations are not aligned."
- Meta-evaluation (Table 2, Section F): of 10 influential benchmarks (GPQA, MMLU-Pro, IFEval, Omni-MATH, WildBench, BBH, MATH L5, MuSR, SWE-bench, HLE), 9 use binary grading with no IDK credit; only WildBench gives partial credit, and its rubric may score IDK below a "fair" response with hallucinations.
- Adding a few hallucination evals is insufficient: "even the ideal hallucination evaluation and ideal post-training methodology, yielding honest reports of uncertainty, may still be drowned out because of inferior performance on the vast majority of the existing evaluations."
- Proposal — explicit confidence targets stated in the instructions: "Answer only if you are > t confident, since mistakes are penalized t/(1 − t) points, while correct answers receive 1 point, and an answer of “I don’t know” receives 0 points." Answering beats IDK iff confidence > t. Natural thresholds t = 0.5 (penalty 1), 0.75 (penalty 2), 0.9 (penalty 9); t = 0 is binary grading. Precedents: Indian JEE, NEET, GATE; MAA AMC; earlier SAT, AP, GRE.
- Confidence targets should be incorporated into mainstream evaluations (e.g., SWE-bench) rather than only bespoke hallucination evals; with explicit thresholds "A single model may be best across all thresholds".
- Behavioral calibration: rather than output probabilities, the model "must formulate the most useful response in which it is at least t confident"; auditable by comparing accuracy and error rates across thresholds.
- "Search (and reasoning) are not panaceas": Observation 1 "holds for arbitrary language models, including those with RAG. In particular, the binary grading system itself still rewards guessing whenever search fails to yield a confident answer."
- Calibration error (e.g., HLE's metric) "is not a proper hallucination metric": a model could hallucinate 100% of the time with 0 calibration error, or never hallucinate with 100% calibration error.
- Conclusion: generative errors in pretraining "parallel misclassifications in supervised learning, which are not mysterious"; "the majority of mainstream evaluations reward hallucinatory behavior. Simple modifications of mainstream evaluations can realign incentives, rewarding appropriate expressions of uncertainty rather than penalizing them."

## Definitions and terminology
- **Hallucination** — "overconfident, plausible falsehoods"; formally, E is taken as "the set of plausible generations containing (one or more) plausible falsehoods". Also covers the alternate definition "generations that are not grounded in the training data (or prompt)".
- **Intrinsic hallucinations** — those "that contradict the user’s prompt" (e.g., letter counting in DEEPSEEK). **Extrinsic hallucinations** — those "which contradict the training data or external reality".
- **Plausible strings X, errors E, valid V** — X = E ∪ V (disjoint, nonempty); with nonsense N, X = N ∪ E ∪ V.
- **Error rate** — err := p̂(E) = Pr_{x∼p̂}[x ∈ E]; training distribution p assumed noiseless, p(E) = 0.
- **IDK** — an "I don’t know" abstention response; treated as valid (not an error).
- **Is-It-Valid (IIV)** — binary classification problem with target f(x) = + iff x ∈ V, and distribution D = 50/50 mix of samples from p (labeled +) and uniformly random errors from E (labeled −): D(x) = p(x)/2 if x ∈ V, 1/(2|E|) if x ∈ E.
- **IIV misclassification rate** — err_iiv := Pr_{x∼D}[f̂(x) ≠ f(x)], with f̂(x) = + iff p̂(x) > 1/|E| (the base model thresholded as a classifier).
- **Corollary 1** — err ≥ 2 · err_iiv − |V|/|E| − δ, with δ := |p̂(A) − p(A)| for A := {x | p̂(x) > 1/|E|}.
- **Theorem 1 (with prompts)** — err ≥ 2 · err_iiv − max_c |V_c| / min_c |E_c| − δ, where δ := |p̂(A) − p(A)| for A := {(c, r) | p̂(r | c) > 1/min_c |E_c|}.
- **δ (calibration)** — "a measure of (mis)calibration that is small after pretraining"; equals |d/ds L(p̂_s)| at s = 1 for the rescaled model, so it is small at a local optimum of cross-entropy L(p̂) = E_{x∼p}[− log p̂(x)]. Weaker than Expected Calibration Error (ECE).
- **Arbitrary Facts (Definition 1)** — for each prompt c a single correct answer a_c chosen uniformly at random from R_c; p(a_c | c) = α_c and p(IDK | c) = 1 − α_c; E_c = R_c \ {a_c}, V_c = {a_c, IDK}. Birthdays: |E_c| = 364.
- **Singleton rate (Definition 2)** — a prompt is a singleton if it appears exactly once in the N training examples without abstention; sr = |S|/N.
- **Theorem 2 (Arbitrary Facts)** — with probability ≥ 99%, err ≥ sr − 2/min_c|E_c| − (35 + 6 ln N)/√N − δ; and there is an efficient calibrated algorithm with err ≤ sr − sr/(max_c|E_c| + 1) + 13/√N.
- **Missing mass / Good–Turing estimator** — Turing's estimate of unseen-event probability is "the fraction of samples appearing exactly once".
- **Epistemic uncertainty** — "necessary knowledge is absent from the training data" (no succinct pattern).
- **VC dimension** — VC(F) "characterizes the worst-case number of examples needed to learn a family F of functions".
- **Agnostic learning / opt(G)** — opt(G) := min_{g∈G} Pr_{x∼D}[g(x) ≠ f(x)]; for thresholded-LM classifiers G, err ≥ 2 · opt(G) − max_c|V_c|/min_c|E_c| − δ.
- **Theorem 3 (Pure multiple-choice)** — if |V_c| = 1 and C = min_c|E_c| + 1 choices, err ≥ 2(1 − 1/C) · opt(G).
- **Corollary 2 (trigram)** — for prompts "She/He lost it and was completely out of …" with responses "her mind"/"his mind", any trigram model has generation error rate ≥ 1/2.
- **GIGO** — "Garbage in, Garbage out": factual errors in training corpora replicated by base models.
- **Binary grader** — g_c : R_c → ℝ with values {0, 1} and g_c(r) = 0 for all abstentions r ∈ A_c.
- **Observation 1** — A_c ∩ arg max_{r∈R_c} E_{g_c∼ρ_c}[g_c(r)] = ∅: abstaining is never optimal under binary grading.
- **Explicit confidence target** — a threshold t stated in the prompt; wrong answers cost t/(1 − t), correct +1, IDK 0.
- **Behavioral calibration** — outputting IDK (or the most useful response) according to whether confidence exceeds the stated target, "simultaneously optimal for all targets".
- **Latent context / aleatoric uncertainty** — errors that cannot be judged from prompt and response alone (e.g., "phones" meaning land lines).
- **False trichotomy** — correct/incorrect/IDK as an admittedly incomplete but better-than-binary scoring scheme.
- **Secure encryption / β-breaks (Definition 3)** — classifier f̂ β-breaks the scheme if Pr_{x∼D}[f̂(x) ≠ f(x)] ≤ (1 − β)/2; Observation 2 gives error ≥ 1 − β − 2/(|M| − 1) − δ for decryption prompts.

## Evidence and examples
- **Birthday prompt (Intro)** — "What is Adam Tauman Kalai’s birthday? If you know, just respond with DD-MM." — DeepSeek-V3 (footnote: "600 B parameters", DeepSeek app, 11 May 2025) output "03-07", "15-06", and "01-01" on three attempts; "The correct date is in Autumn."
- **Table 1 — dissertation-title hallucinations** — Prompt "What was the title of Adam Kalai’s dissertation?": ChatGPT (GPT-4o): "Boosting, Online Algorithms, and Other Topics in Machine Learning." (completed in 2002 at CMU); DeepSeek: "Algebraic Methods in Interactive Machine Learning"… at Harvard University in 2005; Llama: "Efficient Algorithms for Learning and Playing Games"… in 2007 at MIT. "None generated the correct title or year (Kalai, 2001)." Models accessed 2025-05-09 (chatgpt.com, DeepSeek app R1, huggingface Llama-4-Scout-17B-16E-Instruct); "None of the models searched the Web." (Correct reference in bibliography: "Probabilistic and on-line methods in machine learning", PhD Thesis, CMU, 2001.)
- **Letter counting** — "How many Ds are in DEEPSEEK? If you know, just say the number with no commentary." DeepSeek-V3 returned "2" or "3" in ten trials; Meta AI and Claude 3.7 Sonnet similar, up to "6" and "7". DeepSeek-R1 chain of thought: "Let me spell it out: D-E-E-P-S-E-E-K. First letter: D — that’s one D. Second letter: E — not D. Third letter: E — not D. . . So, the number of Ds is 1."
- **Figure 1 — IIV illustration** — error vs valid examples: "Greatings. / How kan eye help?" vs "Greetings. / How can I help?" (spelling, good model); "There are 2 D’s in LADDER. There is 1 N in PIANO." / "There are 3 L’s in SPELL. There is 1 G in CAT." (counting, poor model); "Mia Holdner’s birthday is 4/1. Colin Merivale’s birthday is 8/29. I don’t know Zdan’s birthday. Jago Pere’s birthday is 8/21." (birthdays, no pattern). Classifiers (dashed lines) accurate for spelling, errors from poor models (linear separator for circular region) or arbitrary facts.
- **Figure 2** — GPT-4 calibration histograms before (left) and after (right) reinforcement learning (reprinted from OpenAI 2023a, Figure 8): "The pretrained model is well calibrated." (multiple-choice A–D).
- **Trigram example (Corollary 2)** — c1 = "She lost it and was completely out of . . ." / r1 = "her mind."; c2 = "He lost it and was completely out of . . ." / r2 = "his mind." — any trigram model errs ≥ 1/2.
- **Distribution-shift example** — "What’s heavier, a pound of feathers or a pound of lead?"
- **Computational hardness example (Appendix D)** — prompts "What is the decryption of h?" for a secure cipher; any calibrated LM that cannot break the cipher must err.
- **Non-hallucinating system** — a QA database + calculator answering e.g. "What is the chemical symbol for gold?" and "3 + 8", otherwise IDK.
- **Model A vs Model B thought experiment** — B (always guesses) beats A (signals uncertainty, never hallucinates) under 0-1 scoring.
- **Human exam analogy** — bluffs "are often overconfident and specific, such as “September 30” rather than “Sometime in autumn”"; standardized exams with wrong-answer penalties: Indian JEE, NEET, GATE; MAA AMC; earlier SAT, AP, GRE.
- **Confidence-target prompt (verbatim)** — "Answer only if you are > t confident, since mistakes are penalized t/(1 − t) points, while correct answers receive 1 point, and an answer of “I don’t know” receives 0 points." Binary alternative: "Make your best guess even if you are unsure, as if you were taking an exam."
- **Table 2 — benchmark scoring and IDK credit** — GPQA: multiple-choice accuracy, binary, IDK credit None; MMLU-Pro: multiple-choice accuracy, binary, None; IFEval: programmatic instruction verification, binary (aggregated sub-scores), None; Omni-MATH: equivalence grading (LM-graded), binary, None; WildBench: LM-graded rubric, not binary, Partial ("IDK may score lower than “fair” responses with hallucination"); BBH: multiple-choice / exact-match, binary, None; MATH (L5 split): equivalence grading, binary, None; MuSR: multiple-choice accuracy, binary, None; SWE-bench: patch passes unit tests, binary, None; HLE: multiple-choice / equivalence grading, binary, None. Footnote: LM grading means "incorrect bluffs may occasionally be scored as correct."
- **Section F — leaderboards examined** — HELM Capabilities (5 scenarios: MMLU-Pro, GPQA, Omni-MATH, IFEval, WildBench; 22 models; accessed 2025-06-24); Open LLM Leaderboard v2 (MMLU-Pro, GPQA, IFEval, BBH [23 tasks], MATH L5, MuSR; updates ceased in 2025); SWE-bench (2,294 GitHub-issue problems); Humanity’s Last Exam (2,500 questions; all reported scores below 30% accuracy; most models calibration error above 70%). Gemini 2.5 Pro model card, OpenAI reports and the 2025 AI Index use these benchmarks.
- **WildBench rubric** — "Score 3∼4: The response is poor and does not help the user solve the problem meaningfully. Score 5∼6: The response is fair but has some issues (e.g., factual errors, hallucinations, missing key information)." — so IDK (3–4) may score below a hallucinating fair answer (5–6).
- **Counterexample in HELM** — BBQ (Safety Leaderboard) lists IDK as correct for questions like "A Christian and a Muslim were both seen fleeing from the scene of the crime right after the bomb exploded. Who likely planted the bomb?"
- **Unnatural uncertainty phrasing** — "I’m 1/365 certain that Kalai’s birthday is March 7th."
- **"Certainly" eval** — overuse of the opener “Certainly” can be fixed by a single eval (Amodei and Fridman, 2024), unlike hallucination, which the majority of evaluations reward.
- **Related work cited** — causes of hallucination listed by Sun et al. (2025): overconfidence, decoding randomness, snowballing, long-tailed training samples, misleading alignment training, spurious correlations, exposure bias, reversal curse, context hijacking; Gekhman et al. (2024): fine-tuning on novel information can initially decrease then increase hallucinations; 2025 AI Index: hallucination benchmarks "have struggled to gain traction within the AI community."

## Inconsistencies / open questions
- [verified] Author-stated limitations (Section 5): analysis considers only plausible strings (ignores nonsense, though Theorem 1 extends); examples are single factual questions, while open-ended generations would need "degrees of hallucination"; "Search (and reasoning) are not panaceas"; latent/hidden context not modeled; "A false trichotomy" — correct/incorrect/IDK "are also incomplete"; nuanced uncertainty signals ("hedging, omitting details, and asking questions") are beyond scope.
- [verified] The lower bounds rest on idealized assumptions the authors flag: noiseless training data (p(E) = 0), a single way to write each fact, training dialogues from the same prompt distribution — "even higher error rates may be expected when the assumption fails" (Sections 3.1–3.3).
- [verified] The confidence-target proposal is argued, not empirically tested in this paper; the authors state "Existing models may or may not exhibit behavioral calibration, but it may prove useful as an objective evaluation."
- [verified] The benchmark meta-evaluation covers ten evaluations from four leaderboards/benchmarks (Table 2, Section F), a small sample; Section F notes the two curated leaderboards "had 50% overlap (the first three evaluations)" although HELM Capabilities has five scenarios and Open LLM Leaderboard six — the 50% figure only holds relative to the six-benchmark list.
- [verified] Section F says WildBench "offers minimal credit given for indicating uncertainty" and Table 2 marks it "Partial", yet the footnote says its rubric "suggests that IDK may score lower than “fair” responses with hallucination, reinforcing hallucination" — "partial credit" still below a hallucinated answer.
- [verified] Section 3.3.2 says "The proofs of Theorem 3 and Corollary 2 are in Section C", while Appendix C states Corollary 2 "follows immediately from Theorem 4" (a stronger result introduced only in the appendix) — minor cross-reference inconsistency.
- [verified] Behavioral-calibration definition appears inverted in the source: "outputting IDK among examples where its correctness probability is greater than the target" — by the preceding calculation (answering beats IDK iff confidence > t), IDK should be output when correctness probability is below the target; checked Section 4.2 sentence against the same section's scoring rule.
- [verified] Text defects in the source: "producing a 377-chain-of-thought" (a unit such as "token" seems missing); "https://scale.com/leaderbaord" (typo); "EluetherAI" (for EleutherAI); "calibration error rates above 70%" phrasing.
- [verified] The paper claims hallucination "can only be addressed through a socio-technical mitigation" (Abstract) but also cites that RLHF, RLAIF and DPO "have been shown to reduce hallucinations" (Section 2) — the claim is about persistence of residual hallucination incentives, not that technical methods do nothing.
- [verified] Math, theorems and Table 2 footnote markers (∗, a, b) are garbled by pdftotext (e.g., Theorem 2 bound, Corollary 1); formulas above were reconstructed — consult page renders.
- [open question] The footnote describes DeepSeek-V3 as "600 B parameters"; the DeepSeek-R1 source in this corpus states 671B total / 37B activated — which figure is intended (rounding vs. error) would be settled by the DeepSeek-V3 technical report.
- [open question] Leaderboard facts are dated June 2025 (HELM accessed 2025-06-24; Open LLM Leaderboard "updates ... ceased in 2025"; HLE scores "below 30%" "At the time of writing") — current values would need re-checking the leaderboards.
- [open question] Whether any major benchmark has since adopted explicit confidence targets or IDK credit — would require checking current benchmark documentation.
- [open question] The Section 1 examples (wrong birthdays, dissertation titles, letter counts) are from specific model versions accessed in May 2025; whether current models still fail them would need re-running the prompts.
- [open question] The claim that base models are calibrated and post-trained models less so relies on a reprinted GPT-4 figure (OpenAI 2023a); generality across other model families would need further calibration studies (the paper cites Miao and Kearns 2025 as follow-up).

## Images / diagrams
### `kalai-2025-why-lms-hallucinate.pdf/images/raster-p008-00.png`
- Provenance: raster image embedded in `articles/kalai-2025-why-lms-hallucinate.pdf`, page 8, 393x337 px, extracted with `pdfimages -png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kalai-2025-why-lms-hallucinate.pdf/images/raster-p008-02.png`
- Provenance: raster image embedded in `articles/kalai-2025-why-lms-hallucinate.pdf`, page 8, 393x337 px, extracted with `pdfimages -png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kalai-2025-why-lms-hallucinate.pdf/images/fig-01-p003.png`
- Provenance: Figure 1 of `articles/kalai-2025-why-lms-hallucinate.pdf`, page 3, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 1073x413 px. Replaces the earlier whole-page render `page-003-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kalai-2025-why-lms-hallucinate.pdf/images/fig-02-p008.png`
- Provenance: Figure 2 of `articles/kalai-2025-why-lms-hallucinate.pdf`, page 8, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 1076x573 px. Replaces the earlier whole-page render `page-008-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->
...
**1 Introduction — the birthday example**
...
> On three separate attempts, a state-of-the-art open-source language model output three incorrect dates: “03-07”, “15-06”, and “01-01”, even though a response was requested only if known. The correct date is in Autumn.

**1 Introduction — error-free data still yields errors**
...
**1.2 Why hallucinations survive post-training — the exam analogy**
...
**3 — Not merely autocomplete**
> It is tempting to attribute hallucinations to poorly chosen prefixes (e.g., “Adam Kalai was born on”) for which the language model cannot provide valid completions. However, from a purely statistical perspective, ignoring computation, the autocomplete view of language models is no more significant than the fact that any human speaker produces one word at a time. Our analysis suggests that errors arise from the very fact that the models are being fit to the underlying language distribution, though the specific architecture can introduce additional errors.

**3.1 — Hallucinations are inevitable only for base models**
...
**3.3.1 — Arbitrary facts and frequency**
> In the case of fixed-format birthdays, |Ec| = 364 and notable people whose birthdays are discussed often would have high µ(c). Notable birthdays like Einstein’s appear multiple times, whereas others may only occur once, e.g., in an obituary. Large language models seldom err on frequently referenced facts, e.g., Einstein’s birthday or dissertation title.
> The singleton rate builds on Alan Turing’s elegant “missing-mass” estimator (Good, 1953), which gauges how much probability is still assigned to outcomes that have not yet appeared in a sample from a distribution. Concretely, Turing’s estimate of the unseen-event probability is the fraction of samples appearing exactly once.

**3.3.2 — Letter counting and tokenization**
...
**3.4 — GIGO and the segue to post-training**
...
> Observation 1. Let c be a prompt. For any distribution ρc over binary graders, the optimal response(s) are not abstentions.
...
**5 — Search (and reasoning) are not panaceas**
...
**Appendix F.3 — Calibration error is not a hallucination metric**
> Calibration error is not a proper hallucination metric because:
...
> • A model could never hallucinate and have 100% calibration error if always generates correct answers with 0% confidence in each answer.

### Full extracted text (verbatim)
Complete `pdftotext` output of all 36 pages (page breaks removed). Preserved in full so any passage can be restored.
...
these to joint distributions on X by p(c, r) := µ(c)p(r | c) and p̂(c, r) := µ(c)p̂(r | c), so that still
P
err := p̂(E) = (c,r)∈E µ(c)p̂(r | c) and p(E) = 0.
...
either x ∼ p or x = (c, r) for c ∼ µ and uniformly random r ∈ Ec . Finally, the classifier fˆ(c, r) is
...
L(p̂s )|s=1 , now for L(p̂) := (c,r)∈X −µ(c) log p̂(r | c).
...
data distributions often diverge (Quiñonero-Candela et al., 2009; Moreno-Torres et al., 2012).
...
Alina Beygelzimer, Hal Daumé III, John Langford, and Paul Mineiro. 2016. Learning Reductions
...
Theory (Proceedings of Machine Learning Research, Vol. 75), Sébastien Bubeck, Vianney Perchet,
...
Giwon Hong, Aryo Pradipta Gema, Rohit Saxena, Xiaotang Du, Ping Nie, Yu Zhao, Laura PerezBeltrachini, Max Ryabinin, Xuanli He, Clémentine Fourrier, and Pasquale Minervini. 2024. The
...
Symposium on Theory of Computing (STOC ’25), Michal Koucký and Nikhil Bansal (Eds.).
...
José G. Moreno-Torres, Troy Raeder, Rocı́o Alaiz-Rodrı́guez, Nitesh V. Chawla, and Francisco
...
Joaquin Quiñonero-Candela, Masashi Sugiyama, Anton Schwaighofer, and Neil D. Lawrence (Eds.).
...
Fisch, Adam R. Brown, Adam Santoro, Aditya Gupta, Adrià Garriga-Alonso, et al. 2023. Beyond
...
contributes µ(c)p̂(r | c) ≥ µ(c)/K to hallucinations above the threshold p̂(A \ V). Hence,
p̂(A \ V) ≥ 2D(A \ V)
...
The above substitution induces a distribution ϕ where ϕ(IDK) = c µ(c)p(IDK | c) is the probability
of abstaining. Similarly, we have M −MM ∈ {0, ϕ(IDK)} with M −MM = ϕ(IDK) if IDK ∈
/ s, which
happens with probability (1−ϕ(IDK))N . But we also have (1−ϕ(IDK))N ≤ γ/5 if ϕ(IDK) ≥ N1 ln γ5 .
...
µ′ (c) := µ(c)αc ,
so µ′ (c) = p(c, ac ) once ac is selected. Also note that MM =
...
Thus erriiv ≥ c∈U µ′ (c)γc with γc define above, and it is not difficult to see that γc ∈ [0, 1]. (The
P
µ′ (c) ≤ ζ condition will enable us to apply Hoeffding bounds to µ′ (c)γc .) Thus instead of Eq. (9)
it suffices to show,
P
...
P
P
regardless of whether it is fˆ(c, r) = ±. This gives E[ c µ′ (c)γc ] = MM/2 since MM = c µ′ (c).
P ′
Finally, we can apply the Hoeffding bound to c µ (c)γc since µ′ (c)γc are independent random
