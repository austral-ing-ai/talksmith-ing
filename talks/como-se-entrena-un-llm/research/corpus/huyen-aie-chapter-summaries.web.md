---
source_file: huyen-aie-chapter-summaries
source_type: web-capture
ingested_at: 2026-09-25
---

# Chapter Summaries (chiphuyen/aie-book, `chapter-summaries.md`)

## Provenance
- Original location: web/huyen-aie-chapter-summaries/ (text from `page.md`; `page.md` is 36226 chars with headings, so no `original.html` fallback was needed)
- Format: html (web capture via talksmith:ingest)
- URL: https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md
- Captured at: 2026-09-25T22:39:13Z
- Page title: aie-book/chapter-summaries.md at main · chiphuyen/aie-book · GitHub
- Author / source (if known): Chip Huyen (GitHub user `chiphuyen`, repository `aie-book`, file `chapter-summaries.md` on branch `main`); summaries "taken from the book" (the book's full title is not printed on this page). Repo stats at capture: Fork 2.6k, Star 17.5k; file 233 lines (145 loc), 33.2 KB.
- Date of original (if known): unknown (not stated in source) — no commit date captured ("Latest commit" section is empty); the page references the author's RLHF blog post of May 2023 as the origin of one image.

## Key claims
- Overview: summaries of the 10 chapters of the book; "Some of the summaries might not make sense to readers without having first read the originating chapters".
- Ch. 1: AI engineering emerged as a discipline "thanks to the availability of foundation models"; LLMs arose from language models "thanks to a training approach called self-supervision"; language models incorporated other modalities to become foundation models; AI engineering "evolved out of ML engineering".
- Ch. 1: "Before building an application, an important yet often overlooked question is whether you should build it."
- Ch. 2: "Since most people will be using ready-made foundation models instead of training one from scratch", the chapter skips training details in favor of modeling factors.
- Ch. 2: "A crucial factor affecting a model's performance is its training data. Large models require a large amount of training data, which can be expensive and time-consuming to acquire. Model providers, therefore, often leverage whatever data is available." Curation is often necessary for "specific languages, especially low-resource languages, and specific domains."
- Ch. 2: "The dominating architecture for language-based foundation models is transformer."
- Ch. 2: Model scale is measured by "three key numbers: the number of parameters, the number of training tokens, and the number of FLOPs needed for training"; "The scaling law helps determine the optimal number of parameters and number of tokens given a compute budget." Scaling bottlenecks: "Up until now, scaling up a model generally makes it better. But how long will this continue to be true?"
- Ch. 2: "Due to the low quality of training data and self-supervision during pre-training, the resulting model might produce outputs that don't align with what users want. This is addressed by post-training, which consists of two steps: supervised finetuning and preference finetuning."
- Ch. 2: "Human preference is diverse and impossible to capture in a single mathematical formula, so existing solutions are far from foolproof."
- Ch. 2: Sampling "makes AI models probabilistic" — great for creative tasks, but "this probabilistic nature also causes inconsistency and hallucinations."
- Ch. 3: Stronger models mean higher potential for catastrophic failures; evaluation of open-ended models is hard; "investments in evaluation still lag behind investments in model and application development."
- Ch. 3: Evaluation approaches: language-modeling metrics (perplexity, cross entropy), functional correctness, similarity scores (exact), AI-as-a-judge (subjective); AI judges "should be supplemented with exact evaluation, human evaluation, or both."
- Ch. 3: "Both comparative evaluation and the post-training alignment process need preference signals, which are expensive to collect. This motivated the development of preference models: specialized AI judges that predict which response users prefer."
- Ch. 4: "Not having a reliable evaluation pipeline is one of the biggest blockers to AI adoption"; for most developers the challenge is "no longer in developing models but in selecting the right models".
- Ch. 4: "Public benchmarks are also likely contaminated, as their data is included in the training data of many models."
- Ch. 4: "No perfect evaluation method exists. It's impossible to capture the ability of a high-dimensional system using one- or few-dimensional scores."
- Ch. 5: Prompt engineering is "human–AI communication"; easy to start, which "misleads many into thinking that it's easy to do it well"; asking the model to "slow down and think step by step can yield surprising improvements"; instruction-following opens models to prompt attacks; "no security measurements will be foolproof."
- Ch. 6: RAG was "Originally developed to overcome a model's context limitations"; two steps (retrieve, then generate); success "depends on the quality of its retriever"; term-based (Elasticsearch, BM25) vs embedding-based retrievers.
- Ch. 6: "The RAG pattern can be seen as a special case of agent where the retriever is a tool the model can use"; "An agent is defined by its environment and the tools it can access"; "The more tools you give a model, the more capabilities the model has … However, the more automated the agent becomes, the more catastrophic its failures."
- Ch. 6: "RAG and agents are both prompt-based methods, as they influence the model's quality solely through inputs without modifying the model itself."
- Ch. 7: "In its early days, finetuning was similar to pre-training—both involved updating the model's entire weights"; full finetuning became impractical as models grew; PEFT reduces trainable parameters; quantized training reduces bits per value; LoRA is parameter- and data-efficient and modular; model merging combines models.
- Ch. 7: "finetuning is easy, but getting data for finetuning is hard. Obtaining high-quality annotated data, especially instruction data, is challenging."
- Ch. 8: "What data you need depends not only on your use case but also on the training phase. Pre-training requires different data from instruction finetuning and preferred finetuning"; three core criteria: "quality, coverage, and quantity."
- Ch. 8: "A small amount of high-quality data can outperform a large amount of noisy data"; increasing diversity is key.
- Ch. 8: Synthetic data became practical once "AI could create realistic, complex data"; "synthetic data must be evaluated to ensure its quality"; "you can't automate thinking through what data you want."
- Ch. 9: Inference latency = TTFT (prefill) + TPOT (decode); latency–throughput tradeoff; most impactful techniques: "quantization …, tensor parallelism …, replica parallelism …, and attention mechanism optimization".
- Ch. 10: Common AI application architecture; guardrails can live in several components; observability matters; conversational interfaces enable new user feedback for "the data flywheel"; "AI engineering is moving closer to product"; "Many AI challenges are, at their core, system problems."

## Definitions and terminology
- **Foundation models** — language models that "incorporated other data modalities"; the building block of AI engineering.
- **AI engineering** — discipline of building applications on top of foundation models, evolved from ML engineering.
- **Self-supervision** — training approach enabling the transition from language models to large language models.
- **Model scale (three numbers)** — number of parameters, number of training tokens, number of FLOPs needed for training.
- **Scaling law** — "helps determine the optimal number of parameters and number of tokens given a compute budget."
- **Post-training** — addresses misalignment of the pre-trained model; "consists of two steps: supervised finetuning and preference finetuning."
- **Sampling** — "the process by which a model generates output tokens"; makes models probabilistic.
- **Perplexity / cross entropy** — language-modeling metrics used in evaluation and data processing.
- **AI-as-a-judge** — using an AI model to evaluate responses; subjective, judge-dependent.
- **Preference models** — "specialized AI judges that predict which response users prefer."
- **Comparative evaluation** — ranking models via pairwise "which of the two models is better?" signals (as in chess).
- **Prompt engineering** — "The process of crafting an instruction to get a model to do what you want".
- **RAG** — "a two-step process. It first retrieves relevant information from external memory and then uses this information to generate more accurate responses."
- **Term-based vs embedding-based retrievers** — Elasticsearch/BM25 vs vector-search-based retrieval.
- **Agent** — "defined by its environment and the tools it can access"; AI acts as planner.
- **PEFT** — reduces finetuning memory "by reducing the number of trainable parameters"; **LoRA** — popular PEFT method; **quantized training** — fewer bits per value; **model merging** — combining multiple models into one that works better.
- **Dataset design criteria** — quality, coverage, quantity.
- **TTFT (time to first token)** — influenced by prefilling; **TPOT (time per output token)** — influenced by decoding.
- **Model-level vs inference-service-level optimization** — changing the model (quantization, distillation, attention optimizations) vs changing how it is served (batching, parallelism, prefill/decode decoupling, prompt caching).
- **Data flywheel** — using user feedback as data for continuously improving models.

## Evidence and examples
- **Table 1-3 (Ch. 1)** — "Common generative AI use cases across consumer and enterprise applications": Coding; Image and Video Production (photo/video editing, design / presentation, ad generation); Writing (email, social media and blog posts / copywriting, SEO, reports, memos, design docs); Education (tutoring, essay grading / employee onboarding, upskill training); Conversational Bots (general chatbot, AI companion / customer support, product copilots); Information Aggregation (summarization, talk-to-your-docs / summarization, market research); Data Organization (image search, Memex / knowledge management, document processing); Workflow Automation (travel planning, event planning / data extraction, entry, and annotation, lead generation). Column assignments reconstructed from a flattened capture.
- **Ch. 2 image (rlhf.png)** — "The overall training workflow with pre-training, SFT, and RLHF. Image originally from my RLHF blog post (May 2023)".
- **Figure 3-8 (ai-judge.png)** — "An example of an AI judge that evaluates the quality of an answer given a question."
- **Figure 4-5 (evaluation-process.png)** — "An overview of the evaluation workflow to evaluate models for your application."
- **Ch. 4 host-vs-API axes** — "data privacy, data lineage, performance, functionality, control, and cost".
- **Figure 5-1 (prompt-anatomy.png)** — "A simple example to show to anatomy of a prompt."
- **Figure 6-3 (rag-architecture.png)** — "A high-level view of how an embedding-based, or semantic, retriever works."
- **RAG examples** — "code copilots might need access to entire codebases, and research assistants may need to analyze multiple books."
- **Figure 7-3 (rag-vs-finetune.png)** — "Example application development flows. After simple retrieval (such as term-based retrieval), whether to experiment with more complex retrieval (such as hybrid search) or finetuning depends on each application and its failure modes."
- **Figure 8-3 (model-perf-dataset.png)** — "The performance gain curve with different dataset sizes can help you esti‐ mate the impact of additional training examples on your model's performance."
- **Figure 9-1 (inference-service.png)** — "A simple inference service."
- **Ch. 9 workload examples** — KV caching matters more for long contexts; prompt caching for long overlapping prompt segments or multi-turn conversations; replica parallelism to lower latency at higher cost.
- **Figure 10-10 (aie-architecture.png)** — "A common generative AI application architecture."
- **Ch. 10 example** — "guardrails can be implemented in the inference service, the model gateway, or as a standalone component."

## Inconsistencies / open questions
- [verified] Ch. 4 says host-vs-API pros and cons are outlined "along seven axes", but lists only six: "data privacy, data lineage, performance, functionality, control, and cost" — counted in the Ch. 4 paragraph.
- [verified] Terminology drift: Ch. 2 calls the second post-training step "preference finetuning", Ch. 8 says "preferred finetuning" — same page.
- [verified] Typos/artifacts in source: "One consolidation is that…" (likely "consolation"), "to show to anatomy of a prompt", "esti‐ mate" (soft-hyphen line-break artifact in Figure 8-3 caption).
- [verified] Author-stated caveats: summaries "might not make sense to readers without having first read the originating chapters"; Ch. 2 deliberately "skipped the nitty-gritty training details"; post-training solutions "are far from foolproof"; "No perfect evaluation method exists"; "no security measurements will be foolproof."
- [verified] These are chapter conclusions, not the chapters themselves — most claims are summaries without the underlying numbers or citations (e.g., scaling law and scaling bottlenecks are mentioned but no figures are given).
- [open question] The book's title, publisher, and publication date are not stated on this page — check the repo README or the book itself before citing it by title.
- [open question] "Up until now, scaling up a model generally makes it better" — time-dependent; the page gives no date, so "until now" cannot be anchored without the commit history.
- [verified] Extraction gaps: Table 1-3 is flattened into run-on text with column boundaries lost (consumer vs enterprise columns ambiguous); GitHub navigation/boilerplate ("Skip to content", sign-in prompts, "You can't perform that action at this time.") is interleaved; all figures are image-only (only captions survive); the "Latest commit" block is empty, so no date is captured.

## Images / diagrams
### `huyen-aie-chapter-summaries.web/images/ai-judge.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/ai-judge.png` (PNG image data, 1353 x 540), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/aie-architecture.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/aie-architecture.png` (PNG image data, 1411 x 914), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/evaluation-process.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/evaluation-process.png` (PNG image data, 1262 x 713), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/inference-service.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/inference-service.png` (PNG image data, 1325 x 754), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/model-perf-dataset.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/model-perf-dataset.png` (PNG image data, 1603 x 980), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/prompt-anatomy.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/prompt-anatomy.png` (PNG image data, 1711 x 447), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/rag-architecture.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/rag-architecture.png` (PNG image data, 1209 x 853), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/rag-vs-finetune.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/rag-vs-finetune.png` (PNG image data, 1244 x 796), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `huyen-aie-chapter-summaries.web/images/rlhf.png`
- Provenance: web asset `web/huyen-aie-chapter-summaries/assets/rlhf.png` (PNG image data, 3242 x 1586), captured from https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md. Alt text: "".
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->
...
> "This chapter also explored the thousands of available public benchmarks. Public benchmarks can help you weed out bad models, but won't help you find the best models for your applications. Public benchmarks are also likely contaminated, as their data is included in the training data of many models."
— Chapter 4

> "The RAG pattern can be seen as a special case of agent where the retriever is a tool the model can use. Both patterns allow a model to circumvent its context limitation and stay more up-to-date, but the agentic pattern can do even more than that. An agent is defined by its environment and the tools it can access. In an AI-powered agent, AI is the planner that analyzes its given task, considers different solutions, and picks the most promising one."
— Chapter 6
...
> "In its early days, finetuning was similar to pre-training—both involved updating the model's entire weights. However, as models increased in size, full finetuning became impractical for most practitioners. The more parameters to update during finetuning, the more memory finetuning needs."
— Chapter 7

> "A comment I often hear from practitioners is that finetuning is easy, but getting data for finetuning is hard. Obtaining high-quality annotated data, especially instruction data, is challenging."
— Chapter 7

> "What data you need depends not only on your use case but also on the training phase. Pre-training requires different data from instruction finetuning and preferred finetuning. However, dataset design across training phases shares the same three core criteria: quality, coverage, and quantity. While how much data a model is trained on is what has been grabbing headlines, having high-quality data with sufficient coverage is just as important. A small amount of high-quality data can outperform a large amount of noisy data."
— Chapter 8
...
~~~~~text
# aie-book/chapter-summaries.md at main · chiphuyen/aie-book · GitHub

_Source: <https://github.com/chiphuyen/aie-book/blob/main/chapter-summaries.md>_

[Skip to content](#start-of-content) 

## Navigation Menu

</>[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fchiphuyen%2Faie-book%2Fblob%2Fmain%2Fchapter-summaries.md)Appearance settingsSearch/[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fchiphuyen%2Faie-book%2Fblob%2Fmain%2Fchapter-summaries.md)[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=chiphuyen%2Faie-book)Appearance settings You signed in with another tab or window. Reload to refresh your session. You signed out in another tab or window. Reload to refresh your session. You switched accounts on another tab or window. Reload to refresh your session. Dismiss alert {{ message }} [chiphuyen](/chiphuyen) / ** [aie-book](/chiphuyen/aie-book) ** Public 

- [Notifications](/login?return_to=%2Fchiphuyen%2Faie-book) You must be signed in to change notification settings 
- [Fork 2.6k](/login?return_to=%2Fchiphuyen%2Faie-book) 
- [Star 17.5k](/login?return_to=%2Fchiphuyen%2Faie-book) 
</chiphuyen/aie-book> 
...
[History](/chiphuyen/aie-book/commits/main/chapter-summaries.md)</chiphuyen/aie-book/commits/main/chapter-summaries.md>History233 lines (145 loc) · 33.2 KBmain/
...
- Code
- Blame
233 lines (145 loc) · 33.2 KB[Raw](https://github.com/chiphuyen/aie-book/raw/refs/heads/main/chapter-summaries.md)Copy raw fileDownload raw fileOutlineEdit and raw actions
...
Category Examples of Consumer Use Cases Examples of Enterprise Use Cases Coding Coding Coding Image and Video Production Photo and video editing  
Design Presentation  
Ad generation Writing Email  
Social media and blog posts Copywriting  
SEO  
Reports, memos, design docs Education Tutoring  
Essay grading Employee onboarding  
Employee upskill training Conversational Bots General chatbot  
AI companion Customer support  
Product copilots Information Aggregation Summarization  
Talk-to-your-docs Summarization  
Market research Data Organization Image search  
Memex Knowledge management  
Document processing Workflow Automation Travel planning  
Event planning Data extraction, entry, and annotation  
Lead generation   
...
![](/chiphuyen/aie-book/raw/main/assets/rlhf.png)</chiphuyen/aie-book/blob/main/assets/rlhf.png>
...
![](/chiphuyen/aie-book/raw/main/assets/ai-judge.png)</chiphuyen/aie-book/blob/main/assets/ai-judge.png>
...
![](/chiphuyen/aie-book/raw/main/assets/evaluation-process.png)</chiphuyen/aie-book/blob/main/assets/evaluation-process.png>
...
![](/chiphuyen/aie-book/raw/main/assets/prompt-anatomy.png)</chiphuyen/aie-book/blob/main/assets/prompt-anatomy.png>
...
![](/chiphuyen/aie-book/raw/main/assets/rag-architecture.png)</chiphuyen/aie-book/blob/main/assets/rag-architecture.png>
...
![](/chiphuyen/aie-book/raw/main/assets/rag-vs-finetune.png)</chiphuyen/aie-book/blob/main/assets/rag-vs-finetune.png>
...
![](/chiphuyen/aie-book/raw/main/assets/model-perf-dataset.png)</chiphuyen/aie-book/blob/main/assets/model-perf-dataset.png>
...
![](/chiphuyen/aie-book/raw/main/assets/inference-service.png)</chiphuyen/aie-book/blob/main/assets/inference-service.png>
...
![](/chiphuyen/aie-book/raw/main/assets/aie-architecture.png)</chiphuyen/aie-book/blob/main/assets/aie-architecture.png>
