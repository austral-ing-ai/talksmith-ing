---
source_file: kaplan-2020-scaling-laws.pdf
source_type: article
...
## Provenance
- Original location: articles/kaplan-2020-scaling-laws.pdf
- Format: pdf (30 pages)
- Author / source (if known): Jared Kaplan* (Johns Hopkins University, OpenAI), Sam McCandlish* (OpenAI), Tom Henighan, Tom B. Brown, Benjamin Chess, Rewon Child, Scott Gray, Alec Radford, Jeffrey Wu, Dario Amodei (all OpenAI). *Equal contribution. Contributions as stated: Kaplan and McCandlish led the research; Henighan the LSTM experiments; Brown, Child, Gray and Radford the optimized Transformer implementation; Wu, Chess and Radford the text datasets; Amodei "provided guidance throughout the project." arXiv preprint, cs.LG.
- Date of original (if known): 23 Jan 2020 (arXiv:2001.08361v1, as printed).
...
## Key claims
- Core finding (abstract): "The loss scales as a power-law with model size, dataset size, and the amount of compute used for training, with some trends spanning more than seven orders of magnitude."
- "Performance depends strongly on scale, weakly on model shape": scale = non-embedding parameters N, dataset size D, compute C; depth vs. width etc. matter very little "within reasonable limits."
- Smooth power laws in N, D, C "when not bottlenecked by the other two"; "We observe no signs of deviation from these trends on the upper end, though performance must flatten out eventually before reaching zero loss."
- Fitted laws: L(N) = (N_c/N)^α_N with α_N ≈ 0.076, N_c ≈ 8.8×10^13 non-embedding params; L(D) = (D_c/D)^α_D with α_D ≈ 0.095, D_c ≈ 5.4×10^13 tokens; L(C_min) = (C_c^min/C_min)^α_C^min with α_C^min ≈ 0.050, C_c^min ≈ 3.1×10^8 PF-days.
- "doubling the number of parameters yields a loss that is smaller by a factor 2^−α_N = 0.95."
- Universality of overfitting: performance improves predictably if N and D scale together; penalty depends on N^0.74/D — "every time we increase the model size 8x, we only need to increase the data by roughly 5x to avoid a penalty."
- To avoid overfitting within seed-noise (~0.02 loss): D ≳ (5×10^3) N^0.74; "models smaller than 10^9 parameters can be trained with minimal overfitting on the 22B token WebText2 dataset."
- Universality of training: learning curves follow power laws roughly independent of model size; early training can be extrapolated.
- Transfer: loss on other distributions tracks training-distribution loss "with a roughly constant offset"; generalization depends almost exclusively on in-distribution validation loss, not on training duration or depth.
...
- "Convergence is inefficient": with fixed compute, optimal performance comes from "training very large models and stopping significantly short of convergence"; data requirements grow "very slowly as D ∼ C^0.27."
- Optimal compute allocation: N ∝ C_min^0.73, B ∝ C_min^0.24, S ∝ C_min^0.03 — "As the computational budget C increases, it should be spent primarily on larger models, without dramatic increases in training time or dataset size." For a 10x compute increase, optimal model size grows ~5x, data only ~2x.
- Compute-efficient training should stop ~α_N/α_S ≈ 10% above the converged loss; vs. typical 2% convergence it uses "7.7x fewer parameter updates, 2.7x more parameters, and 65% less compute to reach the same loss."
- Models between 0.6x and 2.2x the optimal size need only ~20% more compute; "A 2.2x larger model requires 45% fewer steps at a cost of 20% more training compute."
- Critical batch size B_crit(L) ≈ B*/L^(1/α_B), B* ≈ 2×10^8 tokens, α_B ≈ 0.21; independent of model size; "approximately doubles for every 13% decrease in loss"; roughly 1–2 million tokens at convergence for the largest models.
- Transformers beat LSTMs asymptotically "due to improved use of long contexts" — LSTMs plateau after <100 tokens.
- Embedding parameters should be excluded from N — "this produces significantly cleaner scaling laws"; suggests the embedding matrix "can be made smaller without impacting performance."
- Contradiction/conjecture: compute-efficient data growth (D ∝ C^0.26–0.27) is slower than overfitting-safe growth (D ∝ C^0.54), so the laws must break down around C* ∼ 10^4 PF-days, N* ∼ 10^12 parameters, D* ∼ 10^12 tokens, L* ∼ 1.7 nats/token — possibly "an estimate of the point at which Transformer language models reach maximal performance" and a rough estimate of the entropy-per-token of natural language.
- Discussion: "Big models may be more important than big data"; "larger models will continue to perform better, and will also be much more sample efficient than has been previously appreciated."
- "Smooth quantitative change can mask major qualitative improvements: 'more is different'."
- Analogy: scaling relations as "analogues of the ideal gas law"; call for a "'statistical mechanics' underlying the 'thermodynamics' we have observed."
- Conjecture that the laws apply to other generative modeling tasks (images, audio, video).
- Learning-rate schedule is "mostly irrelevant" given enough total learning rate, warmup and final decay.

## Definitions and terminology
- **L** — "the cross entropy loss in nats," typically averaged over tokens in a 1024-token context.
- **N** — "the number of model parameters, excluding all vocabulary and positional embeddings"; N ≈ 2·d_model·n_layer·(2·d_attn + d_ff) = 12·n_layer·d_model² with d_attn = d_ff/4 = d_model.
- **C ≈ 6NBS** — estimate of total non-embedding training compute (B = batch size, S = training steps); the factor 6 accounts for forward (≈2N per token) and backward passes. Quoted in PF-days: 1 PF-day = 10^15 × 24 × 3600 = 8.64 × 10^19 floating point operations.
- **C_forward ≈ 2N + 2·n_layer·n_ctx·d_model** — forward-pass add-multiply operations per token.
- **D** — dataset size in tokens.
- **B_crit (critical batch size)** — B_crit(L) ≡ E_min/S_min; "Training at the critical batch size provides a roughly optimal compromise between time and compute efficiency" (2·S_min steps, 2·E_min examples).
- **C_min** — "an estimate of the minimum amount of non-embedding compute to reach a given value of the loss," i.e., training at B ≪ B_crit; C_min(C) = C / (1 + B/B_crit(L)).
- **S_min** — "an estimate of the minimal number of training steps needed to reach a given value of the loss" (B ≫ B_crit); S_min(S) = S / (1 + B_crit(L)/B).
- **α_X** — power-law exponents, L(X) ∝ 1/X^α_X.
- **L(N, D)** — [(N_c/N)^(α_N/α_D) + D_c/D]^α_D (Eq. 1.5); fit α_N = 0.076, α_D = 0.103, N_c = 6.4×10^13, D_c = 1.8×10^13.
- **L(N, S)** — (N_c/N)^α_N + (S_c/S_min(S))^α_S (Eq. 1.6); S_c ≈ 2.1×10^3, α_S ≈ 0.76 (Table 3: α_N 0.077, N_c 6.5×10^13).
- **δL (overfitting)** — L(N, D)/L(N, ∞) − 1 ≈ [1 + (N/N_c)^(α_N/α_D)·D_c/D]^α_D − 1.
- **Compute-efficient frontier** — optimal N, B, S for fixed compute: N ∝ C^(α_C^min/α_N), B ∝ C^(α_C^min/α_B), S ∝ C^(α_C^min/α_S), D = B·S, with α_C^min = 1/(1/α_S + 1/α_B + 1/α_N).
- **Convergence factor f** — L(N, C) = (1 + f)·L(N, ∞); compute-efficient f = α_N/α_S ≈ 10%.
- **Early stopping step** — S_stop(N, D) ≳ S_c / [L(N, D) − L(N, ∞)]^(1/α_S).
- **WebText2** — extended WebText: outbound Reddit links with ≥3 karma through December 2017 plus January–October 2018; 20.3M documents, 96 GB, 1.62×10^10 words, 2.29×10^10 tokens.
- **Byte-pair encoding** — tokenizer used, vocabulary n_vocab = 50257.
- **Universal (recurrent) Transformers** — re-use parameters; slightly better per parameter, slightly worse per FLOP.
- **Jamming transition** — (cited) generalization transition when model size reaches dataset size; not observed here.
- **Compute-efficient trend table (Table 6)** — N_opt = 1.3·10^9 · C_min^0.73 params; B = 2.0·10^6 · C_min^0.24 tokens; S_min = 5.4·10^3 · C_min^0.03 steps; D_opt = 2·10^10 · C_min^0.27 tokens (1 epoch).
- **Learning-rate rule of thumb** — LR(N) ≈ 0.003239 − 0.0001395·log(N) (breaks down for N > 10^10).

## Evidence and examples
- **Setup** — decoder-only Transformers on WebText2, BPE vocab 50257, 1024-token context; Adam for 2.5×10^5 steps, batch 512 sequences × 1024 tokens (2^19 tokens); Adafactor for >1B-parameter models; 3000-step linear warmup + cosine decay to zero; 10% dropout. Test set 6.6×10^8 tokens; also tested on Books Corpus, Common Crawl, English Wikipedia and Internet Books.
- **Ranges varied** — model size 768 to 1.5 billion non-embedding parameters; dataset 22 million to 23 billion tokens; shape; context length; batch size.
- **Figure 1** — test loss vs. compute (PF-days), dataset size (tokens), parameters: smooth power laws; "For optimal performance all three factors must be scaled up in tandem."
- **Figure 2** — models from 10^3 to 10^9 parameters: larger models reach the same loss with fewer tokens; "Compute-efficient training stops far short of convergence."
- **Figure 3** — for a billion-fold compute increase: optimal model size increases >1,000,000x, batch size ~100x, serial steps <10x (garbled labels in extraction).
- **Figure 4 / Figure 9** — early-stopped L(N, D) for data sizes 21M–22.0B tokens; overfitting depends on N^(α_N/α_D)/D. Poor fit only when data is reduced 1024x (~2×10^7 tokens; an epoch = 40 updates).
- **Figure 5** — loss varies only a few percent across shapes; aspect ratio can vary 40x; (n_layer, d_model) = (6, 4288) within 3% of the (48, 1600) model of GPT-2 [RWC+19]; "22% additional compute compensates for 1% loss increase."
- **Figure 6** — including embedding parameters obscures the trend; excluding them collapses different depths onto one line (except <2 layers or extreme ratios).
- **Figure 7** — LSTMs vs. Transformers: "LSTM plateaus after <100 tokens / Transformer improves through the whole context."
- **Figure 8** — generalization to Internet Books, Books, Wikipedia, Common Crawl improves in parallel with WebText2, small slowly-growing offset.
- **Figure 10** — B_crit = 2.1×10^8 tokens · L^−4.8; doubles every 13% loss decrease.
- **Figure 11** — at fixed compute or steps, loss follows L(N, S); each compute budget has an optimal model size.
- **Figure 12** — 0.6x–2.2x optimal size costs ~20% extra compute.
- **Figure 13** — L = (C_min/2.3×10^8)^−0.050 vs. L = (C/2.0×10^7)^−0.057; lump at 10^−5 PF-days = transition from 1- to 2-layer networks.
- **Figure 14** — N = (1.3×10^9)·C_min^0.73 vs. N = (1.6×10^9)·C^0.88 (fixed batch); S_min = (5.4×10^3)·C_min^0.03.
- **Figure 15** — L(C_min) and L(D(C_min)) intersect: "The intersection point is sensitive to the precise power-law parameters."
- **Figure 16** — early-stopping step vs. overfitting; 300M models on subsampled datasets; L_test − L_train overestimates overfitting.
- **Figure 17** — recurrent Transformers with 2x/4x/8x reuse.
- **Figure 19** — sample efficiency improves "by a factor of almost 100 when comparing the smallest possible model to a very large one."
- **Figures 20–21** — per-token loss is a power law in context position T (e.g., 774M model); models learn short-range information first; n_ctx = 8 models beat n_ctx = 1024 models on very early tokens.
- **Figure 22** — learning-rate schedules on a 3M model; run-to-run variation ~0.05 loss.
- **Figure 23** — power law fits L(N) better than a logarithm (L = 0.25·log(N/7.1×10^12)).
- **Figure 24** — ~1.5B-parameter models: no depth effect on generalization; the 12-layer model overfit Internet Books.
- **WebText2 tokenization** — "1.4 tokens per word and 4.3 characters per token" (words by wc).
- **Table 1** — per-operation parameter and FLOP counts for Transformer (embed, QKV, mask, project, feedforward, de-embed).
- **Tables 4–6 (Appendix A)** — summary of power laws: α_N = 0.076, α_D = 0.095, α_C = 0.057 (C_c = 1.6×10^7 PF-days), α_C^min = 0.050 (C_c^min = 3.1×10^8 PF-days), α_B = 0.21 (B* = 2.1×10^8 tokens), α_S = 0.76 (S_c = 2.1×10^3 steps).

## Inconsistencies / open questions
- [verified] The range of scale is stated three different ways: abstract "more than seven orders of magnitude"; Section 1.1 "more than six orders of magnitude"; Section 1.2 "eight orders of magnitude in C_min, six orders of magnitude in N, and over two orders of magnitude in D" — checked all three passages.
- [verified] Fitted constants differ between sections: α_D = 0.095 and D_c = 5.4×10^13 (Eq. 1.2, Table 5) vs. α_D = 0.103 and D_c = 1.8×10^13 (Table 2); N_c = 8.8×10^13 vs. 6.4×10^13 (Table 2) vs. 6.5×10^13 (Table 3); authors note the L(N, D) parameters "differ very slightly" because the full function is fit.
- [verified] The compute exponent varies: empirical α_C^min ≈ 0.050 (Eq. 1.3, Figure 13) vs. predicted ≈ 0.054 (Eq. 6.4) vs. ≈ 0.052 (Eq. B.6); N ∝ C^0.73 (empirical) vs. C^0.71 (predicted) — the authors call these "in excellent agreement" / "within a few percent."
- [verified] Data-with-compute exponent: D ∼ C^0.27 (Summary, Table 6, D_e = 2·10^10) vs. D(C_min) ≈ 4×10^10 tokens·(C_min/PF-day)^0.26 (Eq. 6.7, which assumes C = 2C_min).
- [verified] B* given as ∼2·10^8 (Eq. 1.4, 5.3) and 2.1×10^8 (Figure 10, Table 5).
- [verified] Appendix B.4 contains unresolved placeholders: "The result is shown in Figure X" and "(see Figure Y)"; B.4 also refers to "A.1", "A.6" and "A.9" equations that do not exist under those labels (Appendix A has no numbered equations; the relevant ones are B.x).
- [verified] References [RRBS19a] and [RRBS19b] are the same paper (Rosenfeld et al., "A constructive prediction of the generalization error across scales", 1909.12673) listed twice.
- [verified] Author-stated caveats (Appendix C): no "solid theoretical understanding" of the laws; B_crit predictions uncertain far outside explored loss range; small-data regime not thoroughly investigated (poor fits at smallest D); no experiments with regularization or data augmentation; C ≈ 6NBS ignores n_ctx terms (confounded when n_ctx ≳ 12·d_model); some hyperparameters (initialization scale, momentum) may be untuned; optimal learning rate depends on target loss; did not test higher learning rates for short runs.
- [verified] Author-stated: "we have not optimized regularization (eg the dropout probability) while varying dataset and model size"; the learning-rate rule "breaks down for N > 10^10 parameters."
- [verified] The intersection point values (C* ∼ 10^4 PF-days, N* ∼ 10^12, D* ∼ 10^12 tokens, L* ∼ 1.7 nats/token) are "highly uncertain, varying by an order or magnitude in either direction."
- [verified] Author-stated: "In practice, researchers typically train smaller models for longer than would be maximally compute-efficient because of hardware constraints."
- [verified] Extraction problems: equations (1.1)–(6.8) and Appendix B equations are garbled (superscripts dropped, e.g., "1015" = 10^15, "219" = 2^19, "103" = 10^3); Figure 3 labels are scrambled; Table 4 is flattened beyond reliable reading; a few typos in source ("are are as follows", "datasetset", "order or magnitude", "harware").
- [open question] The recommendation that compute should go "primarily on larger models" with slow data growth (N ∝ C^0.73, D ∝ C^0.27) conflicts with later compute-optimal findings (e.g., Hoffmann et al. 2022, "Chinchilla", also in this corpus, recommends scaling N and D equally) — settle by cross-reading the Hoffmann record, which attributes part of the difference to fixed learning-rate schedules and smaller model ranges.
- [open question] Whether "Big models may be more important than big data" still reflects current practice (2020 claim) — would need current training reports (e.g., overtrained models on 10T+ tokens).
- [open question] The conjectured L* ≈ 1.7 nats/token as the entropy of natural language is speculative and tokenizer-dependent — later scaling studies would settle whether loss levels off there.
- [open question] Whether smooth loss improvements translate into task capabilities is flagged by the authors as open ("it will be important to investigate whether continued improvement on the loss translates into improvement on relevant language tasks").

## Images / diagrams
### `kaplan-2020-scaling-laws.pdf/images/fig-01-p003.png`
- Provenance: Figure 1 of `articles/kaplan-2020-scaling-laws.pdf`, page 3, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 997x436 px. Replaces the earlier whole-page render `page-003-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-02-p004.png`
- Provenance: Figure 2 of `articles/kaplan-2020-scaling-laws.pdf`, page 4, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 987x482 px. Replaces the earlier whole-page render `page-004-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-03-p004.png`
- Provenance: Figure 3 of `articles/kaplan-2020-scaling-laws.pdf`, page 4, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x950 px. Replaces the earlier whole-page render `page-004-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-04-p005.png`
- Provenance: Figure 4 of `articles/kaplan-2020-scaling-laws.pdf`, page 5, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x394 px. Replaces the earlier whole-page render `page-005-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-05-p008.png`
- Provenance: Figure 5 of `articles/kaplan-2020-scaling-laws.pdf`, page 8, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 993x403 px. Replaces the earlier whole-page render `page-008-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-06-p008.png`
- Provenance: Figure 6 of `articles/kaplan-2020-scaling-laws.pdf`, page 8, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 993x825 px. Replaces the earlier whole-page render `page-008-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-07-p009.png`
- Provenance: Figure 7 of `articles/kaplan-2020-scaling-laws.pdf`, page 9; cropped by hand-set bounding box (automatic crop missed it), rendered at 160 dpi, 1037x379 px. Replaces the earlier whole-page render `page-009-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-08-p010.png`
- Provenance: Figure 8 of `articles/kaplan-2020-scaling-laws.pdf`, page 10, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x465 px. Replaces the earlier whole-page render `page-010-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-09-p011.png`
- Provenance: Figure 9 of `articles/kaplan-2020-scaling-laws.pdf`, page 11, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x400 px. Replaces the earlier whole-page render `page-011-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-10-p012.png`
- Provenance: Figure 10 of `articles/kaplan-2020-scaling-laws.pdf`, page 12, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x511 px. Replaces the earlier whole-page render `page-012-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-11-p014.png`
- Provenance: Figure 11 of `articles/kaplan-2020-scaling-laws.pdf`, page 14, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x434 px. Replaces the earlier whole-page render `page-014-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-12-p015.png`
- Provenance: Figure 12 of `articles/kaplan-2020-scaling-laws.pdf`, page 15, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x466 px. Replaces the earlier whole-page render `page-015-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-13-p015.png`
- Provenance: Figure 13 of `articles/kaplan-2020-scaling-laws.pdf`, page 15, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x941 px. Replaces the earlier whole-page render `page-015-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-14-p016.png`
- Provenance: Figure 14 of `articles/kaplan-2020-scaling-laws.pdf`, page 16, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x460 px. Replaces the earlier whole-page render `page-016-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-15-p017.png`
- Provenance: Figure 15 of `articles/kaplan-2020-scaling-laws.pdf`, page 17, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x444 px. Replaces the earlier whole-page render `page-017-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-16-p023.png`
- Provenance: Figure 16 of `articles/kaplan-2020-scaling-laws.pdf`, page 23, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x488 px. Replaces the earlier whole-page render `page-023-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-17-p024.png`
- Provenance: Figure 17 of `articles/kaplan-2020-scaling-laws.pdf`, page 24, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x412 px. Replaces the earlier whole-page render `page-024-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-18-p024.png`
- Provenance: Figure 18 of `articles/kaplan-2020-scaling-laws.pdf`, page 24, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x391 px. Replaces the earlier whole-page render `page-024-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-19-p024.png`
- Provenance: Figure 19 of `articles/kaplan-2020-scaling-laws.pdf`, page 24, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x403 px. Replaces the earlier whole-page render `page-024-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-20-p025.png`
- Provenance: Figure 20 of `articles/kaplan-2020-scaling-laws.pdf`, page 25, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x414 px. Replaces the earlier whole-page render `page-025-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-21-p025.png`
- Provenance: Figure 21 of `articles/kaplan-2020-scaling-laws.pdf`, page 25, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x400 px. Replaces the earlier whole-page render `page-025-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-22-p026.png`
- Provenance: Figure 22 of `articles/kaplan-2020-scaling-laws.pdf`, page 26, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x432 px. Replaces the earlier whole-page render `page-026-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-23-p026.png`
- Provenance: Figure 23 of `articles/kaplan-2020-scaling-laws.pdf`, page 26, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x389 px. Replaces the earlier whole-page render `page-026-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->

### `kaplan-2020-scaling-laws.pdf/images/fig-24-p027.png`
- Provenance: Figure 24 of `articles/kaplan-2020-scaling-laws.pdf`, page 27, vector figure; cropped from the page with PyMuPDF (graphics bbox + labels + caption), rendered at 160 dpi, 988x449 px. Replaces the earlier whole-page render `page-027-figures.png`.
- Depiction: <!-- pending: process_images -->
- Why it matters: <!-- pending: process_images -->
- Transcribed text: <!-- pending: process_images -->
...
Complete `pdftotext` output of all 30 pages (page breaks removed). Preserved in full so any passage can be restored.
...
Here we display predicted compute when using a sufficiently small batch size. See Figure 13 for comparison to the
...
αmin
min
L(Cmin ) = Ccmin /Cmin C ; αC
∼ 0.050, Ccmin ∼ 3.1 × 108 (PF-days)
(1.3)
3

We also observe an empirical power-law trend with the training compute C (Figure 1) while training at fixed batch
...
training set [RWC+ 19]. The power laws αN , αD , αC
expected as we scale up N , D, or Cmin ; for example, doubling the number of parameters yields a loss that
is smaller by a factor 2−αN = 0.95. The precise numerical values of Nc , Ccmin , and Dc depend on the
...
αN
size sublinearly according to D ∝ N αD ∼ N 0.74 . In fact, we find that there is a single equation combining
...
0.24
0.03
which closely matches the empirically optimal results N ∝ Cmin
, B ∝ Cmin
, and S ∝ Cmin
. As the
...
where one PF-day = 1015 × 24 × 3600 = 8.64 × 1019 floating point operations.
...
Cforward ≈ 2N + 2nlayer nctx dmodel
...
nlayer dmodel 3dattn

2nlayer dmodel 3dattn
...
Cforward = 2N + 2nlayer nctx dattn
...
2.29 × 1010 tokens. We reserve 6.6 × 108 of these tokens for use as a test set, and we also test on similarlyprepared samples of Books Corpus [ZKZ+ 15], Common Crawl [Fou], English Wikipedia, and a collection
...
we simultaneously varied dmodel while keeping N ≈ 12nlayer d2model fixed. Similarly, to vary dff at fixed
...
trains.
C
with the best performance on step S = 6BS
. Note that in these results the batch size B remains fixed for
...
αN
see Figure 4.) Right: The extent of overfitting depends predominantly on the ratio N αD /D, as predicted in
...
Section 3, as here we are fitting the full L(N, D) rather than just L(N, ∞) or L(∞, D).
...
For example, one might have used L(N, D) =
...
Although the critical batch size roughly matches the gradient noise scale, we are using a direct measurements of
...
where L(N, ∞) is the converged loss, evaluated with infinite available data. This inequality and its comparison to the empirical data is displayed in Figure 16 in the appendix. In that figure, the values of Sstop
...
One might ask why we did not simply train at Bcrit in the first place. The reason is that it depends not only on the
...
By definition Cmin ≡ 6N Bcrit S, and so we can use N (Cmin ) to extract further results. In particular, since
−0.05
0.24
prior fits show B ∝ L−4.8 and L ∝ Cmin
, we can conclude that Bcrit ∝ Cmin
...
min
Section 5. Given our equation for L(N, Smin ), we can substitute Smin = C
6N B and then find the minimum
...
2Cmin
D(Cmin ) =
≈ 4 × 1010 tokens (Cmin /PF-Day)0.26
(6.7)
6N (Cmin )
...
−0.050
our prediction for L(Cmin ) from Figure 13, where we found a scaling L(Cmin ) ∝ Cmin
.
...
∗
Cmin
and N ∗ , we have extracted all of the reliable information available in natural language data. In this
...
Defining words using the wc utility, the WebText2 dataset has 1.4 tokens per word and 4.3 characters per token.
After this work was completed, [RRBS19a] also appeared, which makes similar predictions for the dependence of
...
(B.5)
αS
which implies that for compute-efficient training, we should train to a fixed percentage ααNS ≈ 10% above
...
There is a slight ambiguity here: we can imagine training either at a constant batch size B (Ltarget ), or we could
instead train at a variable batch size B̃ (L), where B̃ is the instantaneous critical batch size (as opposed to B, which is
...
For compute-efficient training we have f = αN /αS ≈ 10% from the previous section, but researchers
typically use a much smaller value. Here, we choose f 0 = 2% as an estimate. For a fixed value of the loss,
we predict:

1/αN
1+f
...
• We used the estimated training compute C ≈ 6N BS, which did not include contributions proportional to nctx (see Section 2.1). So our scalings with compute may be confounded in practice in the
...
Sample Efficiency vs Model Size
...
Illustration of sample efficiency and compute efficiency. . . . . . . . . . . . . . . . . . . . .
...
Early stopping lower bound and training curves for overfit models . . . . . . . . . . . . . .
...
Another look at sample efficiency . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
Key parameters to trend fits . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

20

6

Trends for compute-efficient training . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
...
Behrooz Ghorbani, Shankar Krishnan, and Ying Xiao. An investigation into neural net optimization via hessian eigenvalue density. CoRR, abs/1901.10159, 2019, 1901.10159. URL
...
Scott Gray, Alec Radford, and Diederik P Kingma. Gpu kernels for block-sparse weights. openai.com, 2017. 19

[HAD19]

Joel Hestness, Newsha Ardalani, and Gregory Diamos. Beyond human-level accuracy: Computational challenges in deep learning. In Proceedings of the 24th Symposium on Principles and
...
Noam Shazeer, Youlong Cheng, Niki Parmar, Dustin Tran, Ashish Vaswani, Penporn Koanantakool, Peter Hawkins, HyoukJoong Lee, Mingsheng Hong, Cliff Young, Ryan Sepassi, and
